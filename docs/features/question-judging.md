...
The running-game **Tools** control opens a responsive modal dialog instead of a compact dropdown. The existing host actions are presented as large action tiles in two columns on wider screens and one column on phones. Actions that lead to another dialog or page dismiss the Tools dialog first. Restart remains available through the same Tools surface, including final-question phases.
