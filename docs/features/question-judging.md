...
During an active regular buzzer question, a player may press **Skip** to propose ending the question without a correct answer. A skip proposal does not create a `QuestionAnswerAttempt`, does not change score, and does not remove the player's buzzer eligibility. The Skip controls appear only while a proposal is available. By default they sit directly under the player name; the adjacent square arrow button moves the entire control pair below the buzzer and changes to an up arrow, and pressing it again restores the original top position. The chosen position is stored for that player/game and reused on later questions and page reloads. The player's proposal is shown as a compact overlay icon on their gameplay card. If that player later presses the buzzer, the proposal is withdrawn immediately and the overlay disappears.
...
The running-game **Tools** control opens a responsive modal dialog instead of a compact dropdown. Host actions are presented as large action tiles in two columns on wider screens and one column on phones. The normal running-game order is **Edit history | Next round**, **Open correct answer | Go to Final Question**, **Blocked players | Show join code**, and **Player achievements | Choose random player**, followed by full-width **Cancel buzzer press**, **Game settings**, and **Restart game** actions. **Previous round** appears as an additional conditional action when available. Actions that lead to another dialog or page dismiss the Tools dialog first. Restart remains available through the same Tools surface, including final-question phases.

If a player claims the buzzer accidentally, the host can use **Cancel buzzer press** from the **Tools** dialog while that claim is active. The command is submitted asynchronously, so the host page stays mounted instead of performing a full-page reload. Cancelling is an undo operation rather than a judgment: it creates no answer attempt, changes no score, clears the answering player and live buzzer race, stops the answer timer, and reopens the buzzer for every still-eligible player including the cancelled claimant. The question timer resumes only when that claim itself paused a running timer; a timer that the host had already paused stays paused. Skip proposals withdrawn by the cancelled winner or accepted late presses are restored.
...
After correct and incorrect outcomes, the host briefly sees the answering player and actual applied score delta over the player-card area. The same successful judging action also plays a host-side synthesized Web Audio feedback cue before the gameplay view refreshes. `Game:CorrectAnswerSound` selects `Triumph` (default), `Arcade`, or `Chime`; `Game:IncorrectAnswerSound` selects `Descent` (default), `ArcadeFall`, or `ChimeFall`; all three use descending pitch sequences, with `Descent` mirroring the `Triumph` note pattern in reverse. Unknown values fall back to the respective defaults. Failed judging commands do not play a feedback cue, and the audio layer does not change scoring or gameplay state. This transient feedback does not delay the authoritative transition to the next gameplay state.
