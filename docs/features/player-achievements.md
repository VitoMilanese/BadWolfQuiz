...
Signing in during a game records the account as a candidate/current-game identity but does not by itself claim old `HostId + normalized nickname` history. The normal automatic proof path remains an eligible finished-game participation with the same host. A host may also explicitly confirm the current signed-in player's identity from the active-game **Player achievements** page; this is a second proof path that links eligible older finished appearances and adopts matching fallback unlock records immediately without treating the current running game as finished or achievement-eligible. Both paths preserve the original achievement code, unlock timestamp, and source game-session ID, including direct event-based achievements, and de-duplicate an achievement by retaining the earliest authoritative unlock. History already claimed by a different account is never reassigned automatically or by host confirmation.

Manual confirmation never trusts an account identifier supplied by the browser. The server verifies that the current host owns the runtime game, resolves the selected player from the game's current players, and obtains that player's account identifier from authoritative runtime state. A nickname with any conflicting prior account claim is not offered for confirmation. After a successful confirmation, the account is immediately re-evaluated against the newly linked eligible `Finished` history so historical metric achievements that now satisfy their thresholds are persisted without waiting for the current game to finish.
...
### Resetting unlocked achievements

Web `1.40.3` allows an unlocked achievement to be reset from the authenticated `/Achievements` page and from the in-game player achievement dialog. Reset is a destructive action and is confirmed through the shared styled dialog rather than a browser-native confirmation prompt.

Reset removes the persisted unlock for every applicable identity of that player, including both the account identity and the host/name fallback when both exist. Host-defined achievement progress for the matching code is cleared as well. The server then stores an internal reset marker for the affected identity; reset markers are implementation details and are never exposed as achievements in the UI.

The marker prevents passive evaluation of historical data from immediately restoring the achievement. A later authoritative qualifying event can earn a built-in achievement again, and an explicit direct unlock also clears the marker. Host-defined achievements count qualifying contributions after the reset boundary until their configured target is reached again. This keeps old history intact for audit/statistics while making the reset meaningful and still allowing the player to re-earn the achievement normally.
...
The active-game **Tools** menu also exposes **Player achievements**, which opens a dedicated host-owned history page. **Unlocked in this game** shows persisted unlocks whose `SourceGameSessionId` belongs to the current stored game. **Players' histories** lets the host select a current player and inspect that player's complete persisted unlock history, newest first. These two history views are read-only. Signed-in players use their account identity; anonymous players use the existing host + normalized-name fallback identity.

A third **Pending confirmation** view is rendered only when at least one current signed-in player has safely adoptable older host/nickname history. It shows the player, the number of eligible finished appearances waiting to be linked, and any visible fallback achievement records whose account copy is missing or has later/weaker metadata. Internal progress-helper rows remain hidden. If old eligible game history needs linking but there is no additional visible achievement card to copy, the player is still shown because linking those games can change account-level historical metrics.

The host may confirm that the current signed-in account belongs to the person who previously used that nickname. The confirmation POST is protected by Razor anti-forgery validation, repeats the owned-game and current-player checks, resolves the account only from runtime state, and refuses nickname histories with conflicting account claims. Confirmation reuses the existing safe finished-history adoption path, never marks the current unfinished game as `Finished` or eligible, preserves old unlock timestamps and source-game IDs, and immediately re-evaluates account metrics against newly linked eligible finished games. When no pending candidates remain, the third tab disappears and the UI moves to the confirmed player's full-history view.

Filter changes, player selection, and a successful confirmation refresh the history surface in place rather than reloading the page. The client preserves scroll position, keeps browser Back/Forward state for normal view navigation, and uses the shared delayed `BadWolfBusy` overlay for slower transitions. After confirmation, both normal views are re-queried from persistence; adopted achievements keep their original source game and therefore do not appear under **Unlocked in this game** unless that source is genuinely the current stored game.

The page uses the browser/page scroll rather than a nested result-list scrollbar, expands to a wider desktop layout, shows larger achievement artwork, centers the history-mode controls, and moves the player selector into a side rail when enough width is available. On narrower screens the selector returns above the history list. `Escape` returns the host directly to the game. Unknown non-internal achievement codes remain displayable with fallback presentation so future host-defined achievements can plug their own metadata/artwork into the same history surface.
