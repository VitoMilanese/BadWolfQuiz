...
2. **Automatically accept new players** — enables or disables automatic approval for newly connected players who would otherwise enter the waiting-for-approval state. This action is hidden when no players are waiting and automatic acceptance is disabled. If automatic acceptance is already enabled, the action remains visible so the host can disable it again.
