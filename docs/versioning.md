...
Web `1.36.0` is a MINOR release that extends first-class question tags to the Final Question Editor. Final questions now use the same localized tag combobox, chips, autocomplete, keyboard interaction, validation, and host-scoped suggestion vocabulary as regular questions. Final-question tags are persisted independently, participate in shared suggestion ranking, are cleared when the final question is deleted, and survive quiz cloning plus `.bwquiz` export/import while remaining compatible with older packages that do not contain the optional final-question tag field. See [`docs/features/question-tags.md`](features/question-tags.md) for the complete authoring and persistence behavior.

Web `1.37.0` is a MINOR release that improves tag authoring across the regular Question Editor, Final Question Editor, and quiz metadata editor. Authors can copy or clear the complete tag list in one action, paste comma- or semicolon-separated tags for bulk insertion, and keep existing duplicate, per-tag length, and quiz-level 20-tag limits intact. The tag autocomplete also consumes `Escape` before the surrounding editor navigation so an open suggestion list closes without leaving the editor. See [`docs/features/question-tags.md`](features/question-tags.md) for the shared authoring behavior.
